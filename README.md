...
- `MV[Answer]` for a short answer question with a math input field that adapts its checking based on whether the answer is an equation or a formula:
  - **Equation answers** (containing `=`): accepts any algebraically equivalent equation, regardless of how terms are rearranged between the two sides. For example, `E/m = c^2`, `m = E/c^2`, and `c^2 = E/m` are all accepted when the correct answer is `E = m*c^2`. The check compares the polynomial numerators of the `LHS - RHS` difference for each equation, so only rearrangements that are equivalent for all non-zero values of the variables are accepted (squaring both sides or other non-invertible transformations are correctly rejected).
  - **Non-equation answers** (expressions without `=`): combines symbolic checking with structural variable-role checking. Mathematically equivalent but structurally different answers are rejected when variables are in different roles (for example `D*x + C` does not match `C*x + D`), while commutative reordering is allowed (for example `D + C*x` matches `C*x + D`).
