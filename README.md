...
- `MV[Answer]` for a short answer question with a math input field that checks for **symbolic equivalence with consistent structural variable roles** while accepting any valid mathematical arrangement. Symbolic simplification is applied before comparison (so for example `M^2*C^2/M` correctly matches `M*C^2`), and any commutative reordering of terms or factors is accepted (for example `D + C*x` matches `C*x + D`). However, answers where variables take structurally different roles are rejected even when mathematically equivalent (for example `D*x + C` does not match `C*x + D`, because `x` is multiplied by a different coefficient in each). For equation answers, terms may additionally be freely rearranged between the two sides or the sides may be swapped (for example `E/m = c^2`, `m = E/c^2`, and `c^2 = E/m` are all accepted when the correct answer is `E = m*c^2`). Same as for `M[Answer]`, `Answer` can include multiple correct answers separated by unescaped `;`.
