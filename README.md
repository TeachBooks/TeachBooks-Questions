...
The placeholder `<Mode>` must be one of the following modes: `T`, `TI`, `TF`, `M`, `MV`, `MEQ`, `MR`, `MNR`, `MAP`, `MRP`, `ME`, `MRE`, `MNRE`, `MAPE`, `MRPE`, `MEQE`. The `<Mode>` defines the input method (text or math), how correctness of an answer is determined and what the placeholder `<Answer>` can be:
...
- `MV[Answer]` for a short answer question with a math input field, which combines symbolic checking with structural variable-role checking. This means mathematically equivalent but structurally different answers are rejected when variables are in different roles (for example `D*x + C` does not match `C*x + D`), while commutative reordering is allowed (for example `D + C*x` matches `C*x + D`).
- `MEQ[Answer]` for a short answer question with a math input field that accepts any algebraically equivalent equation, regardless of how terms are rearranged between the two sides. Both the student answer and `Answer` must be equations (i.e. use `=`). This means, for example, that `E/m = c^2`, `m = E/c^2`, and `c^2 = E/m` are all accepted when the correct answer is `E = m*c^2`. The check compares the polynomial numerators of the `LHS - RHS` difference for each equation, so only rearrangements that are equivalent for all non-zero values of the variables are accepted (squaring both sides or other non-invertible transformations are correctly rejected). Same as for `M[Answer]`, `Answer` can include multiple correct equations separated by unescaped `;`.
...
- `ME[Answer]`, `MRE[Answer]`, `MNRE[Answer]`, `MAPE[Answer]`, `MRPE[Answer]`, and `MEQE[Answer]` behave like `M`, `MR`, `MNR`, `MAP`, `MRP`, and `MEQ` respectively for checking, but additionally format the **shown answer** numerically when the user clicks "Show answer" using a specified number of significant digits.
