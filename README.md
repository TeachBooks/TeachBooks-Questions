...
The placeholder `<Mode>` must be one of the following modes: `T`, `TI`, `TF`, `M`, `MV`, `MR`, `MNR`, `MAP`, `MRP`, `ME`, `MRE`, `MNRE`, `MAPE`, `MRPE`, `MVE`. The `<Mode>` defines the input method (text or math), how correctness of an answer is determined and what the placeholder `<Answer>` can be:
...
- `MV[Answer]` for a short answer question with a math input field designed for answers where **variables must play consistent structural roles** while **any valid mathematical arrangement is accepted**. Concretely, the mode combines symbolic equality checking with structural variable-role checking and adapts its behaviour based on whether the answer is an equation or a formula:
  - **Equation answers** (containing `=`): any algebraically equivalent arrangement of the equation is accepted — terms may be freely rearranged between the two sides or the sides may be swapped. For example, `E/m = c^2`, `m = E/c^2`, and `c^2 = E/m` are all accepted when the correct answer is `E = m*c^2`. The check compares the polynomial numerators of the `LHS - RHS` difference for each equation, so only rearrangements that are equivalent for all non-zero values of the variables are accepted (squaring both sides or other non-invertible transformations are correctly rejected).
  - **Non-equation answers** (expressions without `=`): combines symbolic checking with structural variable-role checking. Any commutative reordering of terms or factors is accepted (for example `D + C*x` matches `C*x + D`), but answers where variables take different structural roles are rejected even when mathematically equivalent (for example `D*x + C` does not match `C*x + D`, because `x` is multiplied by a different coefficient in each).
  - Same as for `M[Answer]`, `Answer` can include multiple correct answers separated by unescaped `;`.
...
- `ME[Answer]`, `MRE[Answer]`, `MNRE[Answer]`, `MAPE[Answer]`, `MRPE[Answer]`, and `MVE[Answer]` behave like `M`, `MR`, `MNR`, `MAP`, `MRP`, and `MV` respectively for checking, but additionally format the **shown answer** numerically when the user clicks "Show answer" using a specified number of significant digits.
