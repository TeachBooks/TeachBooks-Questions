...
The placeholder `<Mode>` must be one of the following modes: `T`, `TI`, `TF`, `M`, `MS`, `MR`, `MNR`, `MAP`, `MRP`, `ME`, `MRE`, `MNRE`, `MAPE`, `MRPE`. The `<Mode>` defines the input method (text or math), how correctness of an answer is determined and what the placeholder `<Answer>` can be:
